...
export function TreatmentSessionRecordCard({ 
  ...
  const canReschedule = sessionRecord.status === 'scheduled' || sessionRecord.status === 'missed';
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(sessionRecord.status)}`}>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            <span className="text-gray-500">📅</span>
            <span className="text-gray-700">{formatDate(sessionRecord.scheduled_date)}</span>
          </div>
          
          <div className="flex items-center gap-2">
            <span className="text-gray-500">🕐</span>
            <span className="text-gray-700">{formatTime(sessionRecord.scheduled_time)}</span>
          </div>
          
          <div className="flex items-center gap-2">
            <span className="text-gray-500">📋</span>
            <span className="text-gray-700">ID da Sessão: {sessionRecord.treatment_session_id}</span>
          </div>
        </div>
        ...
        {sessionRecord.completion_notes && (
          <div className="mt-3 p-3 bg-green-50 rounded-md">
            <p className="text-sm text-gray-600">
              <span className="font-medium">Notas de Conclusão:</span> {sessionRecord.completion_notes}
            </p>
          </div>
        )}
        ...
        {sessionRecord.missed_reason && (
          <div className="mt-3 p-3 bg-red-50 rounded-md">
            <p className="text-sm text-gray-600">
              <span className="font-medium">Motivo da Falta:</span> {sessionRecord.missed_reason}
            </p>
          </div>
        )}
        ...
        {sessionRecord.completion_date && (
          <div className="text-xs text-gray-500 mt-2">
            Completado em: {formatDate(sessionRecord.completion_date)} às {formatTime(sessionRecord.completion_date)}
          </div>
        )}

        {sessionRecord.missed_date && (
          <div className="text-xs text-gray-500 mt-2">
            Marcado como perdido em: {formatDate(sessionRecord.missed_date)} às {formatTime(sessionRecord.missed_date)}
          </div>
        )}
      </div>
    </div>
