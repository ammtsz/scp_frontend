...
import TreatmentSessionConfirmation, { type CreatedTreatmentSession } from '../TreatmentSessionConfirmation';
...
describe('TreatmentSessionConfirmation', () => {
  ...
  it('should render success confirmation with patient name', () => {
    ...
    expect(screen.getByText('Tratamento registrado com sucesso!')).toBeInTheDocument();
    expect(screen.getByText(`Tratamento para ${patientName} foi registrado e os agendamentos foram criados automaticamente.`)).toBeInTheDocument();
  });
  ...
  it('should show automatic scheduling information', () => {
    ...
    expect(screen.getByText('Agendamentos automáticos criados')).toBeInTheDocument();
    expect(screen.getByText(/Todos os agendamentos foram criados automaticamente para as terças-feiras/)).toBeInTheDocument();
    expect(screen.getByText('Agendamentos automáticos (às terças-feiras):')).toBeInTheDocument();
  });
  ...
  it('should handle empty sessions array gracefully', () => {
    ...
    const statisticsContainer = screen.getByText('Séries criadas').closest('.bg-blue-50');
