 ...
 */
const TreatmentSessionConfirmation: React.FC<TreatmentSessionConfirmationProps> = ({
  sessions,
  patientName,
  onAcknowledge,
  customMessage,
  ...
  const lightBathSessions = sessions.filter(s => s.treatment_type === 'light_bath');
  const rodSessions = sessions.filter(s => s.treatment_type === 'rod');

  // Calculate total appointments
  const totalAppointments = sessions.reduce((sum, session) => sum + session.planned_sessions, 0);
  ...
  const renderSessionGroup = (sessionGroup: CreatedTreatmentSession[], title: string) => {
    ...
    return (
      <div className="bg-gray-50 rounded-lg p-4 space-y-3">
        <h4 className="font-medium text-gray-900 flex items-center space-x-2">
          ...
          <span>{title}</span>
          <span className="text-sm text-gray-500">({sessionGroup.length} {sessionGroup.length === 1 ? 'série' : 'séries'})</span>
        </h4>
        ...
        <div className="space-y-3">
          {sessionGroup.map((session) => {
            ...
            return (
              <div key={session.id} className="bg-white rounded border border-gray-200 p-3">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                      {getTreatmentTypeBadge(session.treatment_type)}
                      <span className="text-sm font-medium text-gray-900">{session.body_location}</span>
                    </div>
                        ...
                        {session.color && (
                          <span className="flex items-center space-x-1">
                            <div 
                              ...
                              style={{ backgroundColor: session.color.toLowerCase() }}
                            ></div>
                  ...
                  <div className="text-right">
                    <div className="text-sm font-medium text-blue-600">
                      {session.planned_sessions} {session.planned_sessions === 1 ? 'sessão' : 'sessões'}
                    </div>
      ...
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            {customMessage || `Tratamento para ${patientName} foi registrado e os agendamentos foram criados automaticamente.`}
          </p>
        </div>
      </div>
      ...
      <div className="bg-blue-50 rounded-lg p-4 mb-6">
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <div className="text-sm text-blue-800">{sessions.length === 1 ? 'Série criada' : 'Séries criadas'}</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-blue-600">{totalAppointments}</div>
            <div className="text-sm text-blue-800">Agendamentos automáticos</div>
          </div>
          <div>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="text-sm text-amber-800">
            ...
            <p>
              ...
              começando na próxima terça após a data de início. Os pacientes podem ver 
              seus agendamentos na agenda e realizar check-in normalmente.
            </p>
          </div>
        </div>
      </div>
